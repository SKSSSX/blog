...
> **读图：** 鉴权与审计都在业务 API 完成，对象存储只发短期 URL；前端从不拼长期地址，也不缓存永久链接。
...
权限校验在业务 API 完成，不要指望对象存储桶策略单独扛这件事。
...
图片预览可在 URL 快过期时重取；下载则在点击按钮时再请求，不要在页面加载时批量生成一堆下载链接。
